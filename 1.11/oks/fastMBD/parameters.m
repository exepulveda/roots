...
%
% The main function is MCrestoration.m
% example:
%       load test_data
...
% correlation method. (this step is skipped if doRegistation=0)
%
...
% (this step is skipped if doPSFestimation=0). If doPSFestimation=0 and
% PSFs are not provided, then MCrestoration estimates only shifts between input
% images with subpixel accuracy. This is particularly usefull for superresolution
% (parameter srf>1) 
%
...
% approach and I also multiply gamma by 1.3 after each iteration in step (2).
% In PSFestimaLnoRgrad.m there is a trick used bu Xu, Jia in their paper
% "Two-phase kernel estimation for robust motion deblurring", ECCV 2010, to
% pick only salient edges. This part can be commented out. 
%
% For details see our paper F. Sroubek and P. Milanfar,
% "Robust Multichannel Blind Deconvolution via Fast Alternating Minimization", 
% IEEE Trans. on Image Processing vol. 21, no. 4, April 2012, pp. 1687 - 1700
% or visit zoi.utia.cas.cz/fastdeconv.html
%
...
PAR.gamma_nonblind = 1e2*PAR.gamma;
